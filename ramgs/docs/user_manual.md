...
Use `array[start..end]` syntax to read multiple consecutive array elements in a single command:

```bash
# Read array elements arr[0] through arr[4]
ramgs get arr[0..4]
# Output: arr[0]=v0,arr[1]=v1,arr[2]=v2,arr[3]=v3,arr[4]=v4

# Combined with other variables
ramgs get counter,arr[0..2],status
# Output: counter=c,arr[0]=v0,arr[1]=v1,arr[2]=v2,status=s

# Struct member array
ramgs get data.values[0..3]
# Output: data.values[0]=v0,data.values[1]=v1,data.values[2]=v2,data.values[3]=v3

# With periodic reading
ramgs get -i 100 arr[0..2]
# Output: time:14:23:45.123 arr[0]=v0,arr[1]=v1,arr[2]=v2
```

**Timestamp Output:**

When using `-i` option for periodic reading, each output line is automatically prefixed with a timestamp:

```bash
ramgs get -i 100 counter
# Output: time:14:23:45.123 counter=100
#         time:14:23:45.223 counter=101
#         ...
```
...
A real-time progress bar is displayed during data collection:
```
Collecting... [25/50] [===========>          ] 50%
```
...
The image command generates both a PNG image and a CSV data file:
- CSV file is saved to the same directory as the image
- Filename format: `image_<varnames>_YYYYMMDD_HHMMSS_fff.csv`
- CSV contains timestamps and all sampled variable values

**Notes:**
- Press ESC to interrupt data collection early (image and CSV are still generated with collected data)
...
### 5.12 snapshot - Capture Camera Image

**Syntax:**
```bash
ramgs snapshot [OPTIONS]
```

**Description:**

Capture images from a camera device (USB webcam, laptop built-in camera, etc.). Useful for documenting physical device states during debugging sessions.

**Options:**

| Option | Default | Description |
|--------|---------|-------------|
| `-d, --device <N>` | 0 | Camera device index |
| `-l, --list` | - | List available camera devices |
| `-o, --output <FILE>` | auto | Custom output filename (single capture only) |
| `-i, --interval <MS>` | - | Interval between captures in milliseconds |
| `-c, --count <N>` | 1 | Number of captures (0 = infinite) |

**Output:**
- Images are saved to `ramgs_tmp_imgs/` directory
- Filename format: `snapshot_YYYYMMDD_HHMMSS_fff.png`
- Each captured image path is output to terminal

**Examples:**

```bash
# Single capture from default camera
ramgs snapshot

# Capture from camera device 1
ramgs snapshot -d 1

# List available cameras
ramgs snapshot --list

# Capture 5 images at 1 second intervals
ramgs snapshot -i 1000 -c 5

# Continuous capture every 500ms (press ESC to stop)
ramgs snapshot -i 500

# Custom output filename
ramgs snapshot -o my_photo.png
```

**Output Format (repeated capture):**
```
Press ESC to stop...
[1] ramgs_tmp_imgs/snapshot_20260129_143052_123.png
[2] ramgs_tmp_imgs/snapshot_20260129_143053_125.png
[3] ramgs_tmp_imgs/snapshot_20260129_143054_127.png
```

**Notes:**
- Requires OpenCV (`pip install opencv-python`)
- Press ESC to stop repeated capture
- The `ramgs_tmp_imgs/` directory is created automatically if it doesn't exist

---

### 5.13 recognize - Recognize Panel Elements

**Syntax:**
```bash
ramgs recognize <IMAGE> [OPTIONS]
```

**Description:**

Recognize display panel elements from a photograph. Uses computer vision to detect which segments/icons are illuminated based on a panel design file.

**Arguments:**

| Argument | Description |
|----------|-------------|
| `IMAGE` | Path to the panel photograph |

**Options:**

| Option | Default | Description |
|--------|---------|-------------|
| `-d, --design <FILE>` | (required) | Path to panel design file (.panel.json) |
| `--json` | false | Output in JSON format instead of text |
| `-t, --threshold <N>` | 0.4 | Brightness threshold for on/off detection (0.0-1.0) |

**Output:**

Text format (default):
```
Panel Recognition Result
========================
Image: photo.jpg
Design: panel.panel.json
Threshold: 0.40
Confidence: 0.85
...
```bash
# Basic recognition
ramgs recognize photo.jpg -d panel.panel.json

# Output in JSON format
ramgs recognize photo.jpg -d panel.json --json

# Adjust brightness threshold
ramgs recognize photo.jpg -d panel.json -t 0.5
```

**Notes:**
- Requires OpenCV (`pip install opencv-python`)
- The threshold value affects sensitivity: lower values detect dimmer segments, higher values require brighter segments
- Confidence score indicates how well the image matches the design template

---

### 5.14 gui - Launch GUI Application
...
| `/display <buffer>` | Render display panel image |
| `/designer` | Launch panel designer |
| `/snapshot` | Capture image from camera |
| `/recognize <image>` | Recognize panel elements from image |
...
RAMViewer includes a Panel Designer for creating virtual display panel layouts. This allows you to visualize MCU display buffer data as graphical panel images.

### 7.11.1 Launching Panel Designer

```bash
# Launch empty designer
ramgs designer
...
| Rect   |          Canvas Area                     |   --------    |
| Circle |                                          |   Geometry    |
| Ellipse|          (Design workspace)              |   Style       |
| Polygon|                                          |   Binding     |
| Text   |                                          |               |
+--------+------------------------------------------+---------------+
| Layer Panel                                                       |
...
| - Reference Layer                                                 |
+------------------------------------------------------------------+
|  Status Bar (mouse position, zoom level, selection info)          |
+------------------------------------------------------------------+
```

### 7.11.3 Drawing Tools

| Tool | Shortcut | Description |
|------|----------|-------------|
| Select | V | Select, move, and resize objects |
| Rectangle | R | Draw rectangles |
| Circle | C | Draw circles (drag from center) |
| Ellipse | E | Draw ellipses |
| Polygon | P | Draw polygons (click to add points, double-click to finish) |
| Text | T | Add text labels |

### 7.11.4 Layer System

The designer uses a fixed 3-layer system:

| Layer | Purpose | Exported |
|-------|---------|----------|
| Design | Display objects with data bindings | Yes |
| Background | Static background elements | Yes |
| Reference | Reference image for tracing | No |
...
**Style Properties:**
- Active Border Color: Border color when object is "on"
- Active Fill Color: Fill color when object is "on"
- Inactive Border Color: Border color when object is "off"
- Inactive Fill Color: Fill color when object is "off"
- Border Width: Line thickness

**Data Binding:**
- Logic: OR (any bit = 1) or AND (all bits = 1)
- Bits: List of (byte_index, bit_index) pairs

### 7.11.6 Data Binding Example

To bind an object to byte 0, bit 0 and byte 0, bit 1 with OR logic:

1. Select the object
2. In Property Panel → Data Binding
3. Set Logic to "OR"
4. Click "+ Add Binding"
5. Set Byte: 0, Bit: 0
6. Click "+ Add Binding" again
7. Set Byte: 0, Bit: 1

When the display buffer has either bit set to 1, the object shows in "active" state.

### 7.11.7 Design File Format

Design files use `.panel.json` extension and contain:
- Canvas size and background color
- Layer configurations
- All display objects with geometry, style, and bindings

### 7.11.8 Keyboard Shortcuts
...
## 7.12 Display Command

The `display` command renders a panel design with MCU buffer data to generate images.

### 7.12.1 Basic Usage

```bash
# CLI mode
ramgs display display_buff[0..15] --design panel.panel.json

# Interactive mode
/display display_buff[0..15] -d panel.panel.json
```

### 7.12.2 Buffer Variable Syntax

| Syntax | Description |
|--------|-------------|
| `var[start..end]` | Read bytes from start to end (inclusive) |
| `var[start,count]` | Read count bytes starting from start |

**Examples:**
```bash
# Read bytes 0-15 (16 bytes)
display_buff[0..15]

# Read 16 bytes starting from index 0
display_buff[0,16]
...
### 7.12.3 Continuous Generation

Generate multiple images at intervals:

```bash
# Generate 10 images at 500ms intervals
ramgs display buff[0..7] -d panel.json -i 500 -c 10

# Interactive mode
/display buff[0..7] -d panel.json -i 500 -c 10
```

Press ESC to stop continuous generation.

### 7.12.4 Output

Images are saved to `ramgs_tmp_imgs/` directory with filenames like:
```
display_20260128_143052_123.png
```
...
|---------|------|---------|
| 1.6.0 | 2026-01 | Added snapshot command for camera image capture |
| 1.5.0 | 2026-01 | Added virtual display panel system (display command, designer GUI) |
| 1.4.0 | 2026-01 | Added get command array range read, image command CSV export and progress bar |
