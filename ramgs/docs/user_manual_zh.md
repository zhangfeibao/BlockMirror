...
支持使用 `array[start..end]` 语法一次读取多个连续数组元素：

```bash
# 读取数组元素 arr[0] 到 arr[4]
ramgs get arr[0..4]
# 输出: arr[0]=v0,arr[1]=v1,arr[2]=v2,arr[3]=v3,arr[4]=v4

# 结合其他变量
ramgs get counter,arr[0..2],status
# 输出: counter=c,arr[0]=v0,arr[1]=v1,arr[2]=v2,status=s

# 结构体成员数组
ramgs get data.values[0..3]
# 输出: data.values[0]=v0,data.values[1]=v1,data.values[2]=v2,data.values[3]=v3

# 配合周期读取
ramgs get -i 100 arr[0..2]
# 输出: time:14:23:45.123 arr[0]=v0,arr[1]=v1,arr[2]=v2
```

**时间戳输出:**

使用 `-i` 选项进行周期读取时，每行输出会自动添加时间戳前缀：

```bash
ramgs get -i 100 counter
# 输出: time:14:23:45.123 counter=100
...
image命令会同时生成PNG图片和CSV数据文件：
- CSV文件保存到与图片相同的目录
- 文件名格式: `image_<变量名>_YYYYMMDD_HHMMSS_fff.csv`
- CSV包含时间戳和所有变量的采样值

**说明:**
- 按ESC键可提前中断数据采集 (仍会用已采集的数据生成图片和CSV)
...
### 5.12 display - 渲染显示面板图像

**语法:**
```bash
ramgs display <缓冲区变量> --design <设计文件> [-i <毫秒>] [-c <次数>]
```

**描述:**

读取MCU显示缓冲区数据，根据面板设计文件渲染生成PNG图像。用于可视化MCU的显示状态。

**参数:**

| 参数 | 描述 |
|------|------|
| `缓冲区变量` | 显示缓冲区变量名，支持数组范围语法 |

**选项:**

| 选项 | 必需 | 描述 |
|------|------|------|
| `-d, --design <文件>` | 是 | 面板设计文件路径 (.panel.json) |
| `-i, --interval <毫秒>` | 否 | 采样间隔，用于连续生成 |
| `-c, --count <次数>` | 否 | 生成次数，与 `-i` 配合使用 |

**缓冲区变量语法:**

| 语法 | 描述 |
|------|------|
| `var[start..end]` | 读取从 start 到 end（包含）的字节 |
| `var[start,count]` | 读取从 start 开始的 count 个字节 |
...
# 使用逗号语法
ramgs display lcd_buffer[0,16] -d my_panel.panel.json
...
**输出:**
- 图像保存到 `ramgs_tmp_imgs/` 目录
- 文件名格式: `display_<变量名>_YYYYMMDD_HHMMSS_fff.png`

**说明:**
- 按ESC键可中断连续生成
- 设计文件使用 `ramgs designer` 命令创建

---

### 5.13 designer - 启动面板设计器
...
启动面板设计器GUI，用于创建和编辑虚拟显示面板设计。设计文件可与 `display` 命令配合使用，将MCU缓冲区数据可视化为面板图像。

**参数:**

| 参数 | 描述 |
|------|------|
| `设计文件` | 可选，要打开的 .panel.json 文件 |
...
每个显示对象可以绑定到缓冲区的一个或多个位：
- **OR逻辑**: 任意绑定位为1时对象显示为"激活"状态
- **AND逻辑**: 所有绑定位为1时对象才显示为"激活"状态
...
从摄像头设备（USB摄像头、笔记本内置摄像头等）捕获图像。用于在调试过程中记录物理设备状态。

**选项:**

| 选项 | 默认值 | 描述 |
|------|--------|------|
| `-d, --device <N>` | 0 | 摄像头设备索引 |
| `-l, --list` | - | 列出可用摄像头设备 |
| `-o, --output <文件>` | 自动 | 自定义输出文件名（仅单次拍摄） |
| `-i, --interval <毫秒>` | - | 拍摄间隔（毫秒） |
| `-c, --count <次数>` | 1 | 拍摄次数（0 = 无限） |

**输出:**
- 图像保存到 `ramgs_tmp_imgs/` 目录
- 文件名格式: `snapshot_YYYYMMDD_HHMMSS_fff.png`
- 每张拍摄的图像路径输出到终端
...
**说明:**
- 需要OpenCV（`pip install opencv-python`）
- 按ESC键停止连续拍摄
- 如果 `ramgs_tmp_imgs/` 目录不存在会自动创建

---

### 5.15 recognize - 识别面板元素

**语法:**
```bash
ramgs recognize <图像> [选项]
```

**描述:**

从照片中识别显示面板元素。使用计算机视觉技术，根据面板设计文件检测哪些段码/图标处于点亮状态。
...
| 选项 | 默认值 | 描述 |
|------|--------|------|
| `-d, --design <文件>` | (必需) | 面板设计文件路径 (.panel.json) |
| `--json` | false | 以JSON格式输出（默认为文本格式） |
| `-t, --threshold <值>` | 0.4 | 亮度阈值，用于判断开/关状态 (0.0-1.0) |
...
**说明:**
- 需要OpenCV（`pip install opencv-python`）
- 阈值影响灵敏度：较低的值可检测较暗的段码，较高的值需要更亮的段码
- 置信度分数表示图像与设计模板的匹配程度
...
| `/chart <vars>` | 显示实时图表 |
| `/image <vars>` | 生成静态图表图片 |
| `/display <buffer>` | 渲染显示面板图像 |
| `/designer` | 启动面板设计器 |
| `/snapshot` | 摄像头拍照 |
| `/recognize <image>` | 从图像识别面板元素 |
...
RAMViewer 包含面板设计器，用于创建虚拟显示面板布局。可以将MCU显示缓冲区数据可视化为图形面板图像。
...
**样式属性：**
- 激活状态边框颜色：对象"开启"时的边框颜色
- 激活状态填充颜色：对象"开启"时的填充颜色
- 非激活状态边框颜色：对象"关闭"时的边框颜色
- 非激活状态填充颜色：对象"关闭"时的填充颜色
- 边框宽度：线条粗细

**数据绑定：**
- 逻辑：OR（任意位为1）或 AND（所有位为1）
- 位列表：(字节索引, 位索引) 对的列表

#### 7.11.6 数据绑定示例

将对象绑定到字节0的位0和位1，使用OR逻辑：
...
当显示缓冲区中任一位为1时，对象显示为"激活"状态。

#### 7.11.7 设计文件格式

设计文件使用 `.panel.json` 扩展名，包含：
- 画布尺寸和背景颜色
- 图层配置
- 所有显示对象的几何、样式和绑定信息
...
`display` 命令使用MCU缓冲区数据渲染面板设计生成图像。
...
| 语法 | 描述 |
|------|------|
| `var[start..end]` | 读取从start到end（包含）的字节 |
| `var[start,count]` | 读取从start开始的count个字节 |
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 1.6.0 | 2026-01 | 添加snapshot命令，支持摄像头拍照 |
| 1.5.0 | 2026-01 | 添加虚拟显示面板系统 (display命令、designer设计器) |
| 1.4.0 | 2026-01 | 添加get命令数组范围读取、image命令CSV导出和进度条 |
